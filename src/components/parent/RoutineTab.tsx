...
const DIFFICULTY_OPTIONS = [
  ...
  { value: 'normal',  label: '보통',   color: 'bg-blue-100 text-blue-700' },
  { value: 'hard',    label: '어려움', color: 'bg-orange-100 text-orange-700' },
  { value: 'special', label: '특별',   color: 'bg-purple-100 text-purple-700' },
]
...
export default function RoutineTab({ missions: initial }: Props) {
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <p className="font-black text-brand-text text-xl">📋 루틴 관리</p>
          <p className="text-xs text-gray-400 mt-0.5">미션을 만들고 켜고 끌 수 있어요</p>
        </div>
        <button
          onClick={() => setShowForm((v) => !v)}
          className="bg-brand-blue text-white text-sm font-bold px-4 py-2 rounded-xl shadow-md active:scale-95 transition-all"
        >
          {showForm ? '닫기' : '+ 미션 추가'}
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="bg-white rounded-2xl p-5 shadow-sm flex flex-col gap-4">
          <p className="font-bold text-brand-text">새 미션 만들기</p>
            ...
            <div className="flex flex-wrap gap-2">
              {ICON_OPTIONS.map((ic) => (
                <button
                  ...
                  className={`w-10 h-10 rounded-xl text-2xl flex items-center justify-center transition-all ${form.icon_emoji === ic ? 'bg-brand-blue/20 ring-2 ring-brand-blue' : 'bg-gray-50'}`}
                >
                  {ic}
                </button>
              ))}
            ...
            <div className="flex gap-2 flex-wrap">
              {DIFFICULTY_OPTIONS.map((d) => (
                <button
                  key={d.value}
                  type="button"
                  onClick={() => setForm({ ...form, difficulty: d.value as Mission['difficulty'] })}
                  className={`text-xs font-bold px-3 py-1.5 rounded-full transition-all ${form.difficulty === d.value ? d.color + ' ring-2 ring-offset-1 ring-current' : 'bg-gray-100 text-gray-500'}`}
                >
                  {d.label}
                </button>
              ))}
            ...
            <div className="flex gap-2 flex-wrap">
              {REPEAT_OPTIONS.map((r) => (
                <button
                  ...
                  className={`text-xs font-bold px-3 py-1.5 rounded-full transition-all ${form.repeat_type === r.value ? 'bg-brand-blue text-white' : 'bg-gray-100 text-gray-500'}`}
                >
                  {r.label}
                </button>
              ))}
          ...
          <div>
            ...
            <input
              ...
              className="w-24 border border-gray-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue/40"
            />
          </div>
            ...
            <div className="flex flex-wrap gap-2">
              {CONCEPT_TAGS.map((tag) => (
                <button
                  ...
                  className={`text-xs font-bold px-2.5 py-1 rounded-full transition-all ${form.concept_tag === tag ? 'bg-brand-green text-white' : 'bg-gray-100 text-gray-500'}`}
                >
                  {tag}
                </button>
              ))}
          ...
          <button
            ...
            className="w-full py-3 bg-brand-blue text-white font-bold rounded-2xl shadow-md active:scale-95 transition-all disabled:opacity-50"
          >
      ...
      {/* 활성 미션 */}
      <section>
        <h2 className="text-sm font-bold text-brand-text mb-2">🟢 활성 미션 ({activeMissions.length})</h2>
      ...
      {inactiveMissions.length > 0 && (
        <section>
          <h2 className="text-sm font-bold text-gray-400 mb-2">⚫ 비활성 미션 ({inactiveMissions.length})</h2>
...
function MissionCard({ mission: m, onToggle, loading }: { mission: Mission; onToggle: (m: Mission) => void; loading: boolean }) {
  const diff = { easy: '쉬움', normal: '보통', hard: '어려움', special: '특별' }[m.difficulty]
  return (
    <div className="bg-white rounded-xl px-4 py-3 shadow-sm flex items-center gap-3">
      ...
      <div className="flex-1 min-w-0">
        ...
        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
          ...
          <span className="text-[10px] font-bold text-brand-blue">🪙{m.credit_reward}</span>
          <span className="text-[10px] font-bold text-sky-500">✨{m.exp_reward}EXP</span>
        </div>
      </div>
      <button
        onClick={() => onToggle(m)}
        disabled={loading}
        className={`flex-shrink-0 w-12 h-6 rounded-full transition-all relative disabled:opacity-50 ${m.is_active ? 'bg-brand-blue' : 'bg-gray-200'}`}
      >
  ...
  return (
    <div>
      ...
      <input
        ...
        className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm text-center font-bold focus:outline-none focus:ring-2 focus:ring-brand-blue/40"
      />
    </div>
  )
}
