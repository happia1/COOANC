...
2.  **메인 화면 (레벨별 적응형 UI)**: 
    *   자녀의 경제 교육 레벨(Lv.0~5)에 따라 UI의 복잡도와 노출되는 기능이 동적으로 변화하는 **진화형 UI/UX**를 제공합니다. **현재 구현 범위는 초기 MVP 단계인 Lv.0부터 Lv.2(\'꽃\' 단계)까지이며, 핵심 경제 개념 습득에 집중합니다.** 만 7세 이상부터 적용될 확장 단계(Lv.3~5)의 **세계 일주 항해 세계관**은 미래 계획으로 문서화되어 있습니다. 자세한 내용은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.
...
COOANC의 레벨업 및 스킵 로직은 다음 알고리즘 다이어그램과 같이 동작합니다. 특히 만 7세 이상부터는 부모의 승인과 이해도 퀴즈를 통해 '세계 일주 항해' 세계관으로 전환되는 로직이 포함되어 있습니다.
