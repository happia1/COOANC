...
본 정보 구조도(IA)는 4-11세 자녀를 위한 생활 습관 및 경제 교육 웹 앱인 'COO-ANC (쿠앵크)'의 콘텐츠 및 기능 계층 구조를 정의합니다. 특히 **아동의 실생활 행동을 기반으로 미션을 생성하고, 그 수행 결과를 가상 보상으로 전환하며, 보호자의 승인 절차를 통해 실제 소비 활동으로 연결하는 통합형 행동-보상-소비 연계 시스템**을 핵심으로 합니다. 부모와 자녀 각각의 사용자 경험을 최적화하기 위해 **레벨별 적응형 UI (Adaptive UI)**와 **LangGraph 기반의 3대 AI 에이전트**를 기반으로 설계되었으며, 모듈형 개발의 확장성을 고려하여 핵심 모듈(루틴, 보상, 상점, 소셜)을 중심으로 구성됩니다.
...
2.  **AI 온보딩 (온보딩 에이전트)**: 초기 온보딩 시 **LangGraph 기반 온보딩 에이전트**와의 대화형 설문을 통해 자녀의 기본 루틴(기상 시간, 주말 알람 여부 등)을 설정하고, 평일/주말/방학/기념일 등 연간 일정을 관리합니다. 간단한 질문을 통해 사용자가 대답하면 그 대답을 구조화하여 초기 세팅값을 설정합니다.
3.  **루틴 매니저 (스케줄 에이전트)**: 부모 화면에 플로팅 아이콘으로 AI 챗봇을 제공하며, 클릭 시 대화창 활성화. 챗봇은 미션 설정, 알람 설정, 연간 일정 관리 키워드 리스트를 선제적으로 보여주어 대화형으로 루틴을 수정/관리합니다. **자연어(텍스트), 사진, 음성 등 멀티모달 입력**으로 루틴을 입력하면 **LangGraph 기반 스케줄 에이전트**가 자동으로 루틴 카드를 생성 및 배치하며, 미션 카드는 온오프, 드래그 앤 드롭으로 순서 변경, 삭제/추가 기능이 제공됩니다. 휴무일에는 자동으로 알람 Off를 해주고 캘린더에 주요 일정을 등록해줍니다. 연간 일정에 따라 자녀 앱에 스페셜 팝업이 노출됩니다.
4.  **자녀 계정 생성 및 관리**: 자녀 프로필 생성, 연령 설정, 캐릭터 선택, 미션 설정.
5.  **대시보드**: 자녀 활동 모니터링, Credit/EXP 현황, **AI 행동 분석 요약 및 가이드 (분석 및 코칭 에이전트)** 확인, 오늘의 특이사항 알림. 루틴 패턴 관찰 → 행동 분석 → 부모를 위한 가이드 제안.
...
8.  **커뮤니티 (확장 계획)**: 육아 및 경제 교육 정보 공유, 부모 간 소통을 위한 커뮤니티 기능.2.2. 자녀 사용자 흐름

1.  **로그인**: 부모를 통한 로그인 또는 간편 로그인
2.  **메인 화면 (레벨별 적응형 UI)**: 
    *   자녀 **레벨(EXP)** 에 따라 기능 아이콘이 열리는 **진화형 UI/UX**를 제공합니다. **v3(2026-06 확정)**: 세계관 **정원(0~4)·마을(5~8)·항구(9~12)·바다(13+)** , 단계 이름은 **씨앗·새싹·저축왕·사업가·탐험가·선장** 만(나머지 「레벨 N」). 미션·마켓은 **Lv.0~**, Lv.1 스티커·Lv.5 저금통·Lv.8 보물상자 구현. Lv.2 **캐릭터 꾸미기 레벨 해금은 제거**. [커리큘럼 v3](/docs/product/CURRICULUM.md) 참조.
...
4.  **정직성 유도 (Honesty Check)**: 미션 완료 클릭 시 캐릭터가 "정말 했어? 거짓말하면 내 마음이 아파요..."와 같은 감성 메시지를 띄우고 **캐릭터의 표정 변화(슬픔)를 시각화**하여 정직한 수행 유도.
5.  **이중 보상 시스템**: 미션 완료 보상으로 EXP (캐릭터 성장 및 꾸미기 아이템 해금) 및 Credit (실물 구매 재화) 획득. 저금통에 Credit 보관 및 잔액 확인. EXP는 즉각적인 보상으로 캐릭터 꾸미기 아이템 해금에 사용되며, Credit은 실물 간식/장난감 구매에 사용됩니다.
6.  **마켓**: Credit을 이용한 간식/학용품 검색 및 구매 요청 (자물쇠/활성화 시각화). 부모 승인 시 **딜리버리 화면** 노출.
7.  **실전 커머스**: 자녀의 구매 요청 시 부모 승인 필요. 부모 승인 시 자녀 앱에 "선물이 집으로 오고 있어요!"라는 축하 애니메이션과 **\'배송 중(트럭 애니메이션)\' → \'집 앞 도착(팝업)\' → \'자녀 수령 확인\'으로 이어지는 풀 프로세스 활성화**.
8.  **캐릭터 꾸미기**: EXP를 통한 캐릭터 성장 및 꾸미기 아이템 해금/착용. 홈 탭에서 캐릭터와 상호작용하며, 다양한 아이템을 드래그 앤 드롭으로 착용/해제합니다.
9.  **칭찬 스티커 시스템**: 특별 미션 완료 시 디지털 칭찬 스티커 발행. 자녀가 직접 스티커를 드래그하여 곰돌이 스티커 판에 붙이는 인터랙션 제공.
10. **확장 기능 (미래 계획)**: 
   *   **세계 일주 항해**: 만 7세 이상부터 배의 진화와 랜드마크 탐험을 통한 세계 일주 항해 세계관 적용 (미래 계획). (미래 계획).
    *   **소셜 기능**: 친구 ID 또는 불특정 다수와 순위 경쟁 (랭킹 시스템), 친구에게 크레딧 선물하기, 기부를 통한 \'기부 나무\' 성장 등 소셜 기능 해제 (미래 계획).## 3. 정보 구조 상세

### 3.1. 부모용 대시보드 (Parent UI) - 3탭 구조

| 탭 구분 | 핵심 역할 (Concept) | 주요 구성 요소 |
| :--- | :--- | :--- |
| **1. 홈 (모니터링)** | **우리 아이 현황판** | 자녀별 요약 정보 (이름, 연령, 현재 크레딧, EXP), 미션 달성률, **AI 행동 분석 요약 및 가이드 (분석 및 코칭 에이전트)**, 최근 활동 로그, 오늘의 특이사항 (휴일 등) 알림 |
| **2. 루틴 매니저 (스케줄 에이전트)** | **말 한마디로 끝내는 설정** | **LangGraph 기반 스케줄 에이전트**: AI 챗봇 기반 온보딩 및 루틴 설정, 플로팅 아이콘, 키워드 기반 대화, **멀티모달 입력(사진, 텍스트, 음성)**, 미션 카드 관리 (드래그앤드롭, 온/오프), 연간 일정 관리(휴일, 기념일 등) 및 **캘린더 자동 등록** |
...
#### 3.1.1. 홈 탭 (모니터링)
*   **자녀 카드**: 여러 자녀가 있을 경우 스와이프로 전환하며, 각 자녀의 캐릭터, 이름, 현재 크레딧, EXP 레벨, **현재 레벨(예: Lv.0 씨앗)**을 한눈에 보여줍니다.
*   **경제 EQ 지수**: 자녀의 경제 EQ 지수 및 레벨별 목표 달성 현황을 시각적으로 제공합니다.
*   **미션 달성률**: 주간/월간 미션 달성률 그래프를 통해 자녀의 꾸준함을 시각적으로 보여줍니다.
*   **AI 행동 분석 요약 (분석 및 코칭 에이전트)**: **LangGraph 기반 분석 및 코칭 에이전트**가 자녀의 소비/저축 성향, 미션 완수 패턴 등을 분석하여 간략하게 요약 제공합니다. 루틴 패턴 관찰 → 행동 분석 → 부모를 위한 가이드 제안.
*   **최근 활동 로그**: 자녀의 미션 완료, 크레딧 사용, 마켓 요청 등 최근 활동 내역을 시간 순으로 보여줍니다.
*   **오늘의 특이사항**: 휴일, 방학 등 루틴에 영향을 줄 수 있는 일정을 **스케줄 에이전트**가 자동으로 감지하여 알림으로 제공합니다.
*   **AI 한 줄 가이드**: "오늘은 경제 관념이 쑥쑥 자랐어요! 칭찬 한마디 어떠세요?"와 같은 AI 기반의 짧은 코칭 문구.

#### 3.1.2. 루틴 매니저 탭 (AI)
*   **AI 챗봇 기반 온보딩 (온보딩 에이전트)**: 초기 설정 시 **LangGraph 기반 온보딩 에이전트**와의 대화를 통해 자녀의 기본 루틴(기상 시간, 등원/하원 시간, 주말 루틴 여부 등)을 설문 형태로 입력받아 자동으로 루틴 카드를 생성합니다. 간단한 질문을 통해 사용자가 대답하면 그 대답을 구조화하여 초기 세팅값을 설정합니다.
*   **자연어 루틴 설정 (스케줄 에이전트)**: 부모가 "우리 아이는 8시에 유치원 가고, 4시에 돌아와서 간식 먹어"와 같이 자연어(텍스트), **사진(가정통신문, 식단표 등), 음성 등 멀티모달 입력**으로 루틴을 입력하면 **LangGraph 기반 스케줄 에이전트**가 이를 분석하여 [08:00 등원 준비], [16:00 간식 시간] 등의 루틴 카드를 타임라인에 자동으로 배치합니다. **다중 일정 등록을 지원하며 캘린더에 자동 입력됩니다.**
...
*   **알람 설정**: 기상 알람, 취침 알람 등 자녀의 루틴에 맞춰 알람을 설정하고, 주말/휴일에는 **스케줄 에이전트**가 자동으로 알람을 일시 정지하거나 해제할 수 있는 기능을 제공합니다. 알람 소리는 부모가 직접 선택하거나 업로드할 수 있습니다.
...
### 3.2. 자녀용 인터페이스 (Kids UI) - 레벨별 적응형 UI (Adaptive UI)

자녀의 연령과 레벨에 따라 UI의 복잡도와 노출되는 기능이 동적으로 변화하는 **진화형 UI/UX**를 제공합니다. **현재 구현 범위는 Lv.0부터 Lv.2까지이며, 이 단계에서는 지도 UI가 노출되지 않습니다.** 에이전트가 아이의 레벨, 난이도별 완주율과 성취율을 분석하여 독바(Dock Bar)에 새로운 탭을 추가하는 등 변화를 줍니다. 각 레벨별 UI 해제 규칙은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.
...
| **5. 소셜 (Social, Level 4+)** | **친구와 함께하는 경제 생활** | 크레딧 선물/교환, 랭킹 시스템, 기부 나무 |

#### 3.2.1. 홈 탭 (My Room)
*   **내 캐릭터**: 화면 중앙에 사용자의 캐릭터가 크게 위치하며, EXP 레벨과 성장 상태, **현재 레벨(예: Lv.0 씨앗)**을 시각적으로 보여줍니다.
...
#### 3.2.5. 소셜 탭 (Social, Level 4+)
*   **독바 확장**: 하단 독바에 **'소셜/랭킹 탭'**이 새롭게 추가됨.
*   **상단 UI**: 화면 상단에 **'기부 나무'**가 상시 노출되어 기부 시 나무가 성장하는 시각적 피드백 제공.
...
### 3.3. 자녀용 인터페이스 (Kids UI) - 학령기 모드 (미래 확장 계획)
...
COOANC의 핵심은 **6단계 경제 교육 커리큘럼**입니다. 이 커리큘럼은 아동의 연령과 발달 단계에 맞춰 경제 개념을 자연스럽게 습득하고 실천할 수 있도록 설계되었습니다. 각 레벨의 상세 내용, 평가 원칙, 연령별 진입 경로, 이벤트 트리거 및 UI 해제 규칙은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.

제공해주신 대외비 커리큘럼 맵은 AI 에이전트의 **RAG(Retrieval Augmented Generation) 기반 지식 베이스**로 활용되어, 개인화된 교육 경험을 제공합니다.

### 4.1. 커리큘럼 통합 및 활용 전략

*   **RAG 기반 지식 베이스**: 대외비 커리큘럼의 상세 내용은 데이터베이스에 직접 저장되지 않고, AI 에이전트가 참조하는 **비공개 지식 베이스**로 구축됩니다. 이는 커리큘럼의 보안을 유지하면서도 AI가 풍부한 교육 콘텐츠를 활용할 수 있도록 합니다.
*   **동적 미션 생성 및 추천**: 부모가 자녀의 연령과 수준을 입력하면, **온보딩 에이전트**가 커리큘럼 맵의 해당 단계에 맞는 미션을 **추천**하는 방식으로 노출합니다. 이는 전체 커리큘럼 맵이 한꺼번에 노출되는 것을 방지하고, 개인화된 학습 경로를 제공합니다.
*   **단계별 잠금(Lock) 시스템**: 현재 단계의 미션을 일정 횟수 이상 완수하거나 특정 성취도를 달성해야 다음 단계의 커리큘럼 미션이 해금되도록 설계됩니다. 이는 콘텐츠의 희소성을 유지하고, 아이들의 성취 동기를 부여합니다.
*   **FQ (경제 지능) 스코어링 연동**: 커리큘럼의 각 단계별 핵심 역량(예: 만족 지연, 계획 소비, 저축 습관)과 연동하여 아이의 FQ 스코어를 산정하고, 이를 통해 아이의 경제 교육 성취도를 시각적으로 보여줍니다.


## 5. 서비스 아키텍처 다이어그램

COO-ANC의 서비스 아키텍처는 LangGraph 기반의 AI 에이전트 통합을 통해 더욱 고도화되었습니다. 현재 구현된 시스템과 미래 에이전트 통합 시스템을 다이어그램으로 시각화하여 보여줍니다.

### 5.1. 현재 시스템 아키텍처 (As-Is)

현재 구현된 COO-ANC의 시스템 아키텍처는 다음과 같습니다.

![현재 COO-ANC 시스템 아키텍처](/docs/design/diagrams/CURRENT_SYSTEM_ARCH.png)

### 5.2. 미래 시스템 아키텍처 (To-Be: AI 에이전트 통합)
...
![미래 COO-ANC 시스템 아키텍처 (AI 에이전트 통합)](/docs/design/diagrams/FUTURE_AGENT_ARCH.png)
...
### 5.3. 자녀용 인터페이스 사용자 흐름

![자녀용 인터페이스 사용자 흐름](/docs/design/diagrams/KIDS_FLOW.png)

### 5.4. 부모용 대시보드 사용자 흐름

![부모용 대시보드 사용자 흐름](/docs/design/diagrams/PARENT_FLOW.png)
...
create table child_stats (
  ...
  current_curriculum_level int not null default 1, -- 현재 커리큘럼 레벨 (1~4+)
  created_at            timestamptz default now(),
  updated_at            timestamptz default now()
);
```
...
create table missions (
  ...
  title           text not null,
  description     text,
  curriculum_level int not null default 1, -- 이 미션이 속한 커리큘럼 레벨
  economic_concept text, -- 미션이 다루는 경제 개념 (예: '노동', '교환', '저축')
...
### 8.1. 레벨·세계관 v3 (2026-06 확정, 앱 구현 기준)

> 구버전 6단계(교환사·Lv.3 저축왕·나눔이·투자가) 표는 **폐기**. 레벨 상승은 **미션 EXP** (`exp_to_next_level`).
...
#### 8.2.3. v3 기능 해금 (마을·항구)

*   **마켓·실물 구매**: **Lv.0부터** (구 「Lv.2 교환사」레벨 해금 **없음**). 구매 요청 → 부모 승인 → 배송 상태 UX는 동일.
*   **Lv.1 새싹**: 칭찬 스티커 판 해금.
*   **Lv.5 저축왕 (마을)**: **저금통** — 가용 크레딧·저축 분리, 저축 습관.
*   **Lv.8 사업가 (마을)**: **보물상자** — 영상·미니게임 등 콘텐츠 이용권.
*   **Lv.10 탐험가 (항구, 향후)**: 감정카드·일기.
*   **Lv.12 선장 (항구, 향후)**: 과일·꽃 팔기.
*   **제거**: Lv.2 **캐릭터 꾸미기** 레벨 해금.

#### 8.2.4. (참고) 구버전 RAG·EQ 승급 서사

구 「교환사·Lv.3 저축왕·나눔이·투자가」 단계별 EQ·자동 승급 서술은 [COOANC_master_doc](/docs/engineering/COOANC_master_doc.md) `[LEVEL-002]`~`[LEVEL-005]`에 **레거시**로 남아 있습니다. **앱은 EXP 레벨업만** 사용합니다.


### 6.3. 레벨업 및 스킵 로직 알고리즘 (MVP 및 확장 단계 통합)

**v3** 레벨업은 미션 **EXP** 누적으로 `current_level`이 오릅니다(한 번에 여러 레벨 가능). 아래 다이어그램은 **구버전(꽃·세계 일주 항해)** 흐름 참고용이며, 현재 앱은 [CURRICULUM v3](/docs/product/CURRICULUM.md)를 따릅니다.

![레벨업 및 스킵 로직 알고리즘 (MVP 및 확장 단계 통합)](/docs/design/diagrams/LEVEL_UP_SKIP_FLOW_V2.png)

**[그림 2] COOANC 레벨업 및 스킵 로직 알고리즘 (MVP 및 확장 단계 통합)**
