...
2.  **메인 화면 (레벨별 적응형 UI)**: 
    *   자녀의 경제 교육 레벨(Lv.0~5)에 따라 UI의 복잡도와 노출되는 기능이 동적으로 변화하는 **진화형 UI/UX**를 제공합니다. 초기 MVP 단계(Lv.0~2)에서는 핵심 경제 개념 습득에 집중하며, 만 7세 이상부터는 확장 단계(Lv.3~5)의 **세계 일주 항해 세계관**이 적용됩니다. 자세한 내용은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.
...
10. **확장 기능 (확장 단계에서 해제)**: 
    *   **세계 일주 항해**: 만 7세 이상부터 배의 진화와 랜드마크 탐험을 통한 세계 일주 항해 세계관 적용.
    *   **소셜 기능**: 친구 ID 또는 불특정 다수와 순위 경쟁 (랭킹 시스템), 친구에게 크레딧 선물하기, 기부를 통한 \'기부 나무\' 성장 등 소셜 기능 해제.## 3. 정보 구조 상세
...
자녀의 연령과 레벨에 따라 UI의 복잡도와 노출되는 기능이 동적으로 변화하는 **진화형 UI/UX**를 제공합니다. 에이전트가 아이의 레벨, 난이도별 완주율과 성취율을 분석하여 독바(Dock Bar)에 새로운 탭을 추가하는 등 변화를 줍니다. 각 레벨별 UI 해제 규칙은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.
