...
2.  **메인 화면 (레벨별 적응형 UI)**: 
    *   자녀의 경제 교육 레벨(Lv.0~5)에 따라 UI의 복잡도와 노출되는 기능이 동적으로 변화하는 **진화형 UI/UX**를 제공합니다. 자세한 내용은 [COOANC 경제 교육 커리큘럼](/docs/product/CURRICULUM.md) 문서를 참조하십시오.
...
### 4.2. 레벨업 및 스킵 로직 알고리즘

COOANC의 레벨업 및 스킵 로직은 다음 알고리즘 다이어그램과 같이 동작합니다.

![레벨업 및 스킵 로직 알고리즘](/docs/design/diagrams/LEVEL_UP_SKIP_FLOW.png)

**[그림 2] COOANC 레벨업 및 스킵 로직 알고리즘**
